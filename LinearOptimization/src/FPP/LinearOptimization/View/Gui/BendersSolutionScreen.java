...
public class BendersSolutionScreen extends JPanel {
	...
	public void initializeScreen() {
		...
		if(steps != null && steps.size() > 0) {
			...
			stepsPanel.setBounds(50, 330, 250, 20);
			this.add(stepsPanel);
			...
			stepsPanel.add(stepsButton, BorderLayout.CENTER);
			stepsPanel.add(stepsTextField, BorderLayout.WEST);
			}
		/*
		scrollPaneSolution = new JScrollPane(solutionTable, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		scrollPaneFunction = new JScrollPane(functionTable, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		scrollPaneFunction.setBounds(399, 90, 523, 60);
		this.add(scrollPaneFunction);
		solutionTable.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
		scrollPaneFunction.setBorder(null);
		
		scrollPaneSolution.repaint();
